...
use super::triangle::{Vector3f, Vector4f};
...
impl texture {
    pub fn new(filename: &std::path::Path, desired_channels: u8) -> Self {
        ...
        match desired_channels {
            ...
            _ => panic!("Unsupported desired channel count"),
        }
    }
    pub fn load(&mut self, filename: &std::path::Path, desired_channels: u8) {
        let img = image::ImageReader::open(filename)
            .expect("Failed to open image")
            .decode()
            .expect("Failed to decode image");
