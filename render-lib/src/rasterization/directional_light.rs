...
impl Default for DirectionalLight {
    fn default() -> Self {
        DirectionalLight {
            ...
            z_near: -0.1,
            z_far: -1000.0,
            angular_diameter: 3.0,
            pixel_radius: 0.0,
            z_buffer_width: 8192,
            z_buffer_height: 8192,
            penumbra_mask_width: 0,
            penumbra_mask_height: 0,
            enable_shadow: true,
            enable_pcf_sample_accelerate: true,
...
impl LightTrait for DirectionalLight {
    ...
    fn look_at(&mut self, models: *const Vec<*mut Model>) {
        ...
        let projection = Matrix4f::from_row_slice(&[
            2.0 / self.view_width,
            0.0,
            0.0,
            0.0,
            ...
            -(self.z_near + self.z_far) / (self.z_near - self.z_far),
            0.0,
            0.0,
            0.0,
            -1.0,
        ...
        self.pixel_radius = 0.5
            * f32::max(
                self.view_width / self.z_buffer_width as f32,
                self.view_height / self.z_buffer_height as f32,
            );
        ...
        let thread_num = min(self.z_buffer_height, DIRECTIONAL_LIGHT_MAXIMUM_THREAD_NUM);
        ...
        let depth_transformer =
            move |z: f32, w: f32| 0.5 * (z * (z_near - z_far) + (z_near + z_far));
        ...
        for tid in 0..thread_num - 1 {
            let handle = thread::spawn(move || unsafe {
                ...
                for model in &*models {
                    ...
                    model_ref.rasterization_shadow_map::<false>(
                        z_buffer_ptr_raw,
                        tid * rows_per_thread,
                        0,
                        rows_per_thread,
    ...
    fn generate_penumbra_mask(&mut self, scene: *const crate::rasterization::scene::Scene) {
        ...
        let penumbra_mask_thread_num =
            min(penumbra_mask_height, DIRECTIONAL_LIGHT_MAXIMUM_THREAD_NUM);
        let rows_per_thread =
        ...
        let light_ptr = MutDirectionalLightPtr(self as *mut DirectionalLight);
...
impl DirectionalLight {
    ...
    pub fn set_angular_diameter(&mut self, angular_diameter: f32) {
        self.angular_diameter = angular_diameter;
    }
    pub fn get_angular_diameter(&self) -> f32 {
    ...
    fn in_shadow_pcf(&self, point: Vector3f, normal: Vector3f) -> f32 {
        ...
        if self.enable_pcf_sample_accelerate {
            let sample_count_inverse = 1.0 / DIRECTIONAL_LIGHT_SAMPLE_NUM as f32;
            for i in 0..DIRECTIONAL_LIGHT_SAMPLE_NUM {
                let sample_dir = compute_fibonacci_spiral_disk_sample_uniform(
                    i,
                    sample_count_inverse,
                    DIRECTIONAL_LIGHT_FIBONACCI_CLUMP_EXPONENT,
                    0.0,
                );
            ...
            unshadow_num as f32 / DIRECTIONAL_LIGHT_SAMPLE_NUM as f32
        } else {
    ...
    fn in_shadow_pcss(&self, point: Vector3f, normal: Vector3f) -> f32 {
        ...
        let penumbra = (block_depth - transform_pos.z) * light_size_div_distance;
        let pcf_radius = f32::max(1.0, (0.25 * penumbra / self.pixel_radius).round()) as i32;
