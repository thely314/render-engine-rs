...
void Triangle::rasterization(const Eigen::Matrix<float, 4, 4> &mvp,
  ...
  crop_triangles_zNear(triangles, scene.zNear);
  crop_triangles_zFar(triangles, scene.zFar);
...
void crop_triangles_zNear(std::vector<Triangle> &triangles, float zNear) {
...
void crop_triangles_zFar(std::vector<Triangle> &triangles, float zFar) {
  const int iter_size = triangles.size();
  int remain_size = 0;
  for (int i = 0; i != iter_size; ++i) {
    int vertex_out_of_box_cnt = 0;
